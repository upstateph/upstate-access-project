...
3. **Access tracks income.** The lowest-income third of tracts averages a shorter
   ...
   neighborhood against county demographics: income, race/ethnicity, and (with
   the next data refresh) households without a vehicle.
...
departures sampled every 10 minutes across an hour, because a single departure instant is
...
data and labeled as such, modeled estimates rather than observed trips, and the full
...
verified, because for those categories accuracy is a safety issue. The rollup design
