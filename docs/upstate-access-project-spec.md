...
Set up the repo structure (Section 8). Write scripts to pull and cache FARS, Census ACS, and the Smart Growth America context data. No PII involved, so it is a safe starting point.
...
Geocoding + Greenlink GTFS routing (walk + wait + transfer time). This is the hardest technical piece, so isolate it so it can be tested independently before wiring it into any UI.
...
- **Data pipeline:** Python, with `pandas` for Census/FARS, a GTFS library (e.g. `gtfs-kit` or `partridge`) for Greenlink parsing, a geocoding library or API for address lookups
